    ...
    return (
      <Container>
        ...
        <RightContainer id="right-container">
          ...
          <Content>
            <Switch>
              <Route exact path="/helper/0" render={() => (
                <div>
                  <h1>{this.state.navList[0].name}</h1>
                  <SegmentBar />
                  <p>码全智能视频图像服务器系统软件集图像标注、图像训练和图像测试等功能于一体。根据用户类型不同，可以分为普通用户、编辑用户、管理用户和超级用户四个拥有不同权限的用户版本。登录网址：<a href="http://192.168.0.118:8000/">http://192.168.0.118:8000/</a>
                  </p>
                  <img src={require("./imgs/0_0_0.png")} className="w3-image"/>
                  <p className="w3-center">图1-1</p>
                </div>
              )} />
              <Route exact path="/helper/1" render={() => (
                <div>
                  <h1>{this.state.navList[1].name}</h1>
                  <SegmentBar />
                  <p>任务管理模块包含：新建任务，删除任务，分发任务，任务训练，查看训练状态和任务测试。系统会根据用户的权限，开放相应的管理权限。</p>
                </div>
              )} />
              <Route exact path="/helper/2" render={() => (
                <div>
                  <h1>{this.state.navList[2].name}</h1>
                  <SegmentBar />
                  <p>图片标注模块包含：图片浏览，图片上传和删除，画框标注，图片操作，标签管理和标注统计。系统会根据用户的权限，开放相应的功能。</p>
                </div>
              )} />
              <Route exact path="/helper/3" render={() => (
                <div>
                  <h1>{this.state.navList[3].name}</h1>
                  <SegmentBar />
                  <p>服务器用户管理模块包含四种用户权限，普通用户，编辑用户，管理用户和超级用户。系统会根据用户的权限，开放相应的功能。</p>
                </div>
              )} />
              <Route exact path="/helper/1/0" render={() => (
                <div>
                  ...
                  <img src={require("./imgs/1_0_1.jpg")} className="w3-image"/>
                  <p className="w3-center">图2-1</p>
                  <p>在图2-1中点击新建任务按钮，出现如图2-2界面： </p>
                  <img src={require("./imgs/1_0_2.jpg")} className="w3-image"/>
                  <p className="w3-center">图2-2</p>
